...
public class AuthenticationResource {
    ...
    public ResponseEntity<UserResponse> updateUser(@Valid @RequestBody RegistrationRequest request , @PathVariable("id") Long idUser) throws UserExistedException, ObjectNotFoundException {
        Optional<User> userOptional = userRepository.findByUsername(request.getUsername());
        if (userOptional.isPresent()) {
            throw new UserExistedException("Username is existed");
        }
        UserResponse userResponse = userService.updateUser(request , idUser);
